(function(){
  ...
  async function render(){
    ...
    const oldCalendar=$('#mealCalendarFixed');if(oldCalendar)oldCalendar.remove();
    const oldCalendar2=$('#casaRecentMeals');if(oldCalendar2)oldCalendar2.remove();
  ...
  async function save(){
    ...
    const del=await client.from('meal_log').delete().eq('meal_date',date).eq('person',person);if(del.error)return window.toast?.(del.error.message);
    if(rows.length){const ins=await client.from('meal_log').insert(rows);if(ins.error)return window.toast?.(ins.error.message)}
    window.toast?.('Comidas guardadas 🌸');if(window.renderAll)window.renderAll();
  }
  ...
  function boot(){style();render();const obs=new MutationObserver(()=>{render()});obs.observe(document.body,{attributes:true,attributeFilter:['class'],subtree:true});}
